...
func TestGremlinQueryIntegration(t *testing.T) {
	...
	rawFrame, err := defaultFrameCodec.ConvertToRawFrame(queryFrame)
	require.Nil(t, err)
	...
	frameContext := &frameDecodeContext{frame: rawFrame, compression: primitive.CompressionNone}
	requestInfo, err := buildRequestInfo(frameContext, []*statementReplacedTerms{}, psCache, metricHandler, "", common.ClusterTypeOrigin, false, false, false, generator)
	require.Nil(t, err)
