...
type PreparedStatementCache struct {
	cache       *simplelru.LRU[md5digest, PreparedEntry] // Map containing the prepared queries (raw bytes) keyed on prepareId
...
func NewPreparedStatementCache(maxSize int) (*PreparedStatementCache, error) {
	indexTargetMap := make(map[targetPreparedId]md5digest)
	indexOriginMap := make(map[originPreparedId]md5digest)

	cache, err := simplelru.NewLRU[md5digest, PreparedEntry](maxSize, func(key md5digest, value PreparedEntry) {
		// this is called by LRU.Add() so we already have a lock here
		// clean up both reverse-lookup indexes for the evicted entry
		preparedData := value.GetPreparedData()
		if preparedData != nil {
			targetId := preparedData.GetTargetPreparedId()
			if targetId != nil {
				delete(indexTargetMap, targetPreparedId(targetId))
			}
			originId := preparedData.GetOriginPreparedId()
			if originId != nil {
				delete(indexOriginMap, originPreparedId(originId))
			}
		}
	})
	if err != nil {
	...
	return &PreparedStatementCache{
		cache:       cache,
		indexTarget: indexTargetMap,
		indexOrigin: indexOriginMap,
		lock:        &sync.RWMutex{},
	}, nil
}
...
func (psc *PreparedStatementCache) GetPreparedStatementCacheSize() float64 {
	...
	log.Debugf("PS Cache Size: %v, PS Target Index Size: %v, PS Origin Index Size: %v.",
		cacheLen, len(psc.indexTarget), len(psc.indexOrigin))
...
func (psc *PreparedStatementCache) StorePreparedOnBoth(
	...
	existingEntry, _ := psc.cache.Get(clientPreparedId)
	...
	psc.cache.Add(clientPreparedId, entry)
...
func (psc *PreparedStatementCache) GetByClientPreparedId(clientPreparedId md5digest) (PreparedEntry, bool) {
	psc.lock.Lock()
	defer psc.lock.Unlock()
	data, ok := psc.cache.Get(clientPreparedId)
	return data, ok
}
...
func (psc *PreparedStatementCache) validateIndexEntry(
	...
	if indexMap != nil && preparedQueryId != nil {
		...
		if ok && clusterClientPreparedId != clientPreparedId {
			...
			psc.cache.Remove(clusterClientPreparedId)
		}
	}
...
func (psc *PreparedStatementCache) storeOneClusterOnly(
	...
	clientPreparedId := psc.computePreparedId(prepareRequestInfo)
	existingEntry, ok := psc.cache.Get(clientPreparedId)
